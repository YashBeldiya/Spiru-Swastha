...
const Media = () => {
    ...
    const categories = ['categoryImage', 'categoryIcon', 'product', 'benefit', 'ingredients', 'howToUse', 'productVideo'];
    ...
    const createFallbackImage = (text = 'Image Not Found') => {
        const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="300" height="200" viewBox="0 0 300 200">
        <rect width="300" height="200" fill="#e0e0e0"/>
        <text x="150" y="100" font-family="Arial" font-size="14" fill="#666" text-anchor="middle">
          ${text}
        </text>
      </svg>
    `;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(response.data.message || 'Error uploading files');
            }
        } catch (error) {
        ...
        if (window.confirm('Are you sure you want to delete this media item?')) {
            try {
                ...
                if (response.data.success) {
                    ...
                    // Handle server-side failure (like media not found)
                ...
                if (error.response) {
                    ...
                    alert(`Error deleting media: ${error.response.data.message || 'Unknown error'}`);
                } else if (error.request) {
                    console.error('Delete error - no response:', error.request);
    ...
    const renderMediaPreview = (media) => {
        ...
        const fileType = media.category;
        const fallbackImage = createFallbackImage(`${fileType} Not Found`);

        if (fileType === 'productVideo') {
            return (
                <div className="relative w-full h-full bg-gray-100 flex items-center justify-center">
                    ...
                    <div class="w-full h-full flex items-center justify-center">
                      <img src="${fallbackImage}" alt="Video not available" class="w-full h-full object-cover" />
                    </div>
                  `;
                        }}
                    >
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                        ...
                        { label: 'Images', value: mediaItems.filter(m => m.category === 'product').length, icon: '🖼️', color: 'blue' },
                        { label: 'Videos', value: mediaItems.filter(m => m.category === 'productVideo').length, icon: '🎥', color: 'purple' },
                                                ...
                                                <div className="flex items-center">
                                                    <div className="flex-shrink-0 h-12 w-12 rounded-lg overflow-hidden">
                                                        <img
                                                            ...
                                                            onError={(e) => {
                                                                e.target.onerror = null;
                                                                e.target.src = createFallbackImage('Image Not Found');
                                                            }}
                                                        />
                                                    </div>
                                        ...
                                        <label className="flex flex-col w-full h-32 border-2 border-blue-300 border-dashed rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100">
                                            <div className="flex flex-col items-center justify-center pt-7">
                                                ...
                                                <p className="text-xs text-blue-400 mt-1">
                                                    Supports: Images (JPEG, PNG, GIF, SVG) and Videos (MP4)
                                                </p>
                                            </div>
                                            <input
                                                type="file"
                                                name="media"
                                                multiple
                                                accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml,video/mp4,video/quicktime,video/x-msvideo,video/webm"
                                                onChange={(e) => {
                    ...
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedMedia(null)}>
                        <div className="bg-white rounded-xl p-6 max-w-4xl max-h-[90vh] overflow-auto border border-blue-200" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="mb-4 flex justify-center">
                                {(selectedMedia.category) === 'productVideo' ? (
                                    <video
                                        controls
